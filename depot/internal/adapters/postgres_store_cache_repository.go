...
func (r PostgresStoreCacheRepository) Add(ctx context.Context, storeID, name, location string) error {
	const query = `INSERT INTO %s (id, name, location)
...
func (r PostgresStoreCacheRepository) Find(ctx context.Context, storeID string) (*domain.Store, error) {
	const query = `SELECT name, location FROM %s
				   WHERE id = $1  LIMIT 1`
	store := &domain.Store{
		ID: storeID,
	}
