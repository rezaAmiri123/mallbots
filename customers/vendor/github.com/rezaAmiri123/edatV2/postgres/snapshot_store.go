...
func (s SnapshotStore) Load(ctx context.Context, aggregate es.EventSourcedAggregate) error {
	const query = `SELECT stream_version, snapshot_name, snapshot_data 
		FROM %s 
	...
	err := s.db.QueryRowContext(ctx, s.table(query),
		aggregate.ID(),
...
func (s SnapshotStore) Save(ctx context.Context, aggregate es.EventSourcedAggregate) error {
	const query = `INSERT INTO %s 
		...
		UPDATE SET stream_version = EXCLUDED.stream_version, 
	...
	if !ok {
		return fmt.Errorf("%T does not implemant es.Snapshotter", aggregate)
	}
...
func (s SnapshotStore) shouldSnapshot(aggregate es.EventSourcedAggregate) bool {
	var maxChange = 3 // low for demonstration; production envs should use higher values 50, 75, 100...
	var pendingVersion = aggregate.PendingVersion()
	var pendingChanges = len(aggregate.Events())

	return pendingVersion >= maxChange && ((pendingVersion >= maxChange) ||
		(pendingVersion%2 < pendingChanges) ||
		(pendingVersion%maxChange == 0))
}
