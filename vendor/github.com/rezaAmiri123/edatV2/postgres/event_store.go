...
func (s EventStore) Load(ctx context.Context, aggregate es.EventSourcedAggregate) (err error) {
	const query = `SELECT stream_version, event_id, event_name, event_data, occurred_at 
		FROM %s 
...
func (s EventStore) Save(ctx context.Context, aggregate es.EventSourcedAggregate) (err error) {
	...
	placeholsers := make([]string, len(aggregate.Events()))
	...
	for i, event := range aggregate.Events() {
		...
		placeholsers[i] = fmt.Sprintf("($%d, $%d, $%d, $%d, $%d, $%d, $%d)",
	...
	_, err = s.db.ExecContext(ctx,
		fmt.Sprintf("%s %s", s.table(query), strings.Join(placeholsers, ",")),
		values...,
	)
	if err != nil {
		return err
